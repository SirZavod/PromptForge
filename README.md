...
- **In-app guide** — press **F1** or click **❓ Guide** anywhere in the app for a built-in walkthrough: first-run order of operations, how subfolders/Tools/LoRA Manager/the queue actually work, and the ComfyUI "last active workflow tab" behavior below. Multi-language from the start (English is complete; other languages are explicitly marked if still pending translation, never silently shown as English without saying so).
