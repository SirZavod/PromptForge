...
class PromptForgeApp:
    ...
    def refresh_library_list(self):
        ...
        def insert_node(parent_iid, parent_path, node):
            for folder_name in sorted(node["subfolders"].keys(), key=str.lower):
            ...
            for base, display_name, content in sorted(node["entries"], key=lambda e: e[1].lower()):
                preview = content.replace("\n", " ")
                preview_short = (preview[:60] + "…") if len(preview) > 60 else preview
                self.tree_library.insert(parent_iid, "end", iid=base, text=display_name, values=(preview_short,))
