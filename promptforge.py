...
class ImageDropZone(tk.Canvas):
    ...
    # The zone's height is driven as a PERCENTAGE of the Entry Editor
    # panel's own height, not a fixed pixel value — that's what makes it
    # look right from a 1080p laptop to a 4K monitor without retuning.
    # The slider moves between MIN_PERCENT and MAX_PERCENT; absolute
    # MIN_PX/MAX_PX simply guard against degenerate tiny/huge windows.
    MIN_PERCENT = 12
    MAX_PERCENT = 65
    DEFAULT_PERCENT = 38
    MIN_PX = 130
    ...
    def __init__(self, master, colors, on_file_chosen, percent=None, **kwargs):
        ...
        self._pil_image = None       # full-res PIL.Image currently loaded (or None)
        self._tk_image = None        # PhotoImage kept alive for Canvas display
        self._has_image = False
        self._last_panel_height = 0  # set by the owner's first apply_panel_height call
    ...
    def set_percent(self, percent):
        """Updates the target percentage (called by the size slider). The
        actual pixel height is recomputed on the next apply_panel_height
        call, which the owner triggers from the panel's <Configure>."""
    ...
    def apply_panel_height(self, panel_height):
        """Recomputes and applies this zone's pixel height as a percentage
        of the Entry Editor panel's total height. Called whenever the
        panel resizes (window resize, maximize, 4K, etc.) or the slider
        moves."""
        self._last_panel_height = panel_height
        target = int(panel_height * (self.percent / 100.0))
        target = max(self.MIN_PX, min(self.MAX_PX, target))
    ...
    def show_placeholder(self):
        """Resets the zone to its empty state."""
    ...
    def show_image_path(self, path):
        """Loads and displays the image at `path`. Falls back to the
        placeholder if the file is missing or unreadable."""
    ...
    def _draw_image(self, w, h, max_area_ratio=0.60):
        c = self.colors
        margin = 6
        self._round_rect(margin, margin, w - margin, h - margin, radius=18,
                          fill=c["bg_card"], outline="")
        ...
        # Cap the image to at most `max_area_ratio` of the zone's area while
        # always preserving aspect ratio. We do this by first fitting the
        # image fully inside the box (the usual "contain" scale), then, if
        # that still exceeds the area budget, scaling the whole box down
        # uniformly until the resulting image area matches the budget.
...
class PromptForgeApp:
    ...
    def build_custom_template_form(self, name):
        ...
        if slot_indices:
            ...
            for idx in slot_indices:
                ...
                combo_char = AutocompleteCombobox(row, textvariable=char_var, width=20)
                ...
                if idx in parsed["outfit_idx"]:
                    ...
                    outfit_combo = AutocompleteCombobox(row, textvariable=outfit_var, width=20)
    ...
    def add_character_slot(self):
        ...
        combo_char = AutocompleteCombobox(row2, textvariable=char_var, width=22)
        combo_char["values"] = ["None"] + self.get_file_list("characters")
        combo_char.current(0)
        combo_char.pack(side="left", padx=(0, 14))
        ...
        combo_outfit = AutocompleteCombobox(row2, textvariable=outfit_var, width=22)
